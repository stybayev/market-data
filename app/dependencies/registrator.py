...
dependencies_container: dict[Type | Callable, Callable] = {}  # type: ignore


def add_factory_to_mapper(class_: Type | Callable) -> Callable[[Callable], Callable]:  # type: ignore  # noqa: E501
    """Регистрирует фабричную функцию для указанного класса или функции.

    Args:
        class_: Класс или функция, для которой регистрируется фабрика.  # noqa: RST306

    Returns:
        Декоратор, который добавляет фабричную функцию в контейнер зависимостей.
    """
    def decorator(factory: Callable) -> Callable:  # type: ignore
