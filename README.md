...
* Отдельный воркер (`app/subscription_manager`) слушает команды из Redis Pub/Sub и поддерживает текущее состояние подписок пользователей.
* Поддерживаемые операции: `subscribe` (добавление), `unsubscribe` (удаление), `set` (полная замена набора тикеров).
* Состояние хранится в Redis (множества `subs:{user_id}`), лимиты по количеству тикеров контролируются на уровне сервиса и вебсокета.
* Запуск в dev-среде — сервис `subscription-manager` в `docker-compose.dev.yml` (использует общий образ приложения).
